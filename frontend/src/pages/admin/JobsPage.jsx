            ...
            {jobs.map((job) => (
              <article key={job.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold leading-4 ${statusClasses(
                      job.status
                    )}`}
                  >
                ...
                <Link
                  to={`/jobs/${job.id}`}
                  className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[var(--brand-700)]"
                >
                  View Progress
